...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t Jonathas-Borges/intranet-trems-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t Jonathas-Borges/intranet-trems-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-trems-backend-acceptance -d Jonathas-Borges/intranet-trems-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-trems-frontend-acceptance --link intranet-trems-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d Jonathas-Borges/intranet-trems-frontend:acceptance
